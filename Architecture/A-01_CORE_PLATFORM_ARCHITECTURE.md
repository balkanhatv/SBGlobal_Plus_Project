...
Client (L6) → L5 API (tRPC/REST)
  1 AuthN: verify IdP token → exchange for platform context token (→ A-03 §3)
...
- A module exposes a **typed service contract** (TS interface) and **domain events**; consumers depend on the contract, never on another module's tables (no cross-module SQL joins across ownership boundaries).
...
NestJS (Core service) · tRPC router mounted in Next.js BFF calling Core (first-party) · REST/OpenAPI controller layer in Core (external) (ADR-005) · PostgreSQL as the single canonical store (→ A-05) · No message broker at v1: Postgres outbox + dispatcher (ADR-006, upgrade seam to a broker recorded in A-12).
