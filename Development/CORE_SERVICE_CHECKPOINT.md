# CORE SERVICE CHECKPOINT — DEV-AI-RAG-SOURCE-READ-001
...
`AIRAGSRC-PG-001`…`AIRAGSRC-PG-007` prove exact immutable source read, sibling-Industry isolation, same-Tenant Tenant-Core visibility, scope-based non-principal-private read behavior, foreign-Tenant/PLATFORM_GLOBAL isolation, safe missing/malformed/route-mismatch behavior, exact bigint-text source version preservation, and absence of mutation/document-revalidation/ACL/retrieve/embed/search/execute methods on the new port.

## Remaining scope

Current DocumentMeta/ACL/scan/sensitivity/residency revalidation, current/latest source selection, source dereference, chunk listing/currentness, chunking-policy execution, embedding/model selection, vector search/retrieval/ranking/filtering/grounding, prompt composition/injection-defense execution, provider/model routing, inference/RAG and public mutation/runtime APIs remain unimplemented unless separately source-owned.
