...
|---|---|---|---|
| Student / `edu_sms_student` | student_no text, principal_id uuid?, admission_id uuid?, class_id uuid?, section_id uuid?, guardian_refs jsonb, status enum(APPLICANT,ENROLLED,ACTIVE,TRANSFERRED,GRADUATED,WITHDRAWN) | PK id; UNIQUE(context,student_no); index context+status+updated_at | SENSITIVE_PERSONAL; student-record policy |
| AdmissionApplication / `edu_sms_admission` | application_no text, applicant_name text, class_id uuid, source text?, state enum(ENQUIRY,APPLICATION,VERIFICATION,ASSESSMENT,OFFERED,WAITLISTED,ENROLLED,REJECTED,WITHDRAWN), submitted_at timestamptz | PK id; UNIQUE(context,application_no); index context+parent/reference | SENSITIVE_PERSONAL; student-record policy |
| ClassSection / `edu_sms_class_section` | session_id uuid, class_code text, section_code text, capacity int CHECK >0, homeroom_principal_id uuid?, status enum(ACTIVE,INACTIVE) | PK id; UNIQUE(context,session_id,class_code,section_code); active/effective partial index | INTERNAL/CONFIDENTIAL; policy retention |
| AttendanceEntry / `edu_sms_attendance` | student_id uuid, attendance_date date, period_code text?, state enum(PRESENT,ABSENT,LATE,EXCUSED), marked_by uuid | append-only where history/evidence; UNIQUE(context,student_id,attendance_date,period_code) | SENSITIVE_PERSONAL; evidentiary retention |
...
| Entity / table | Domain fields (in addition to baseline) | Constraints / indexes | Sensitivity / retention |
|---|---|---|---|
| Program / `edu_cum_program` | code text, name text, department_id uuid, level text, duration_terms int, status enum(ACTIVE,INACTIVE) | PK id; UNIQUE(context,code); index context+status+updated_at | SENSITIVE_PERSONAL; academic-record policy |
| Enrollment / `edu_cum_enrollment` | student_ref uuid, program_id uuid, term_id uuid, enrollment_no text, state enum(APPLIED,VERIFIED,ADMITTED,REGISTERED,ACTIVE,COMPLETED,WITHDRAWN) | PK id; UNIQUE(context,enrollment_no); index context+parent/reference | SENSITIVE_PERSONAL; academic-record policy |
| CourseOffering / `edu_cum_course_offering` | course_code text, term_id uuid, faculty_principal_id uuid?, capacity int, credits numeric, status enum(PLANNED,OPEN,CLOSED) | PK id; UNIQUE(context,term_id,course_code); active/effective partial index | INTERNAL/CONFIDENTIAL; policy retention |
| AcademicProgress / `edu_cum_progress` | enrollment_id uuid, term_id uuid, credits_attempted numeric, credits_earned numeric, gpa numeric?, standing text, finalized_at timestamptz? | append-only where history/evidence; UNIQUE(context,enrollment_id,term_id) | SENSITIVE_PERSONAL; evidentiary retention |
...
| Entity / table | Domain fields (in addition to baseline) | Constraints / indexes | Sensitivity / retention |
|---|---|---|---|
| Enquiry / `edu_ctm_enquiry` | enquiry_no text, source text, counselor_id uuid?, prospect_name text, state enum(NEW,CONTACTED,COUNSELING,DEMO,OFFERED,CONVERTED,LOST) | PK id; UNIQUE(context,enquiry_no); index context+status+updated_at | SENSITIVE_PERSONAL; training-record policy |
| BatchEnrollment / `edu_ctm_enrollment` | student_ref uuid, batch_id uuid, fee_plan_ref uuid?, state enum(OFFERED,ENROLLED,ATTENDING,COMPLETED,DROPPED), enrolled_at timestamptz | PK id; UNIQUE(context,student_ref,batch_id); index context+parent/reference | SENSITIVE_PERSONAL; training-record policy |
| TrainingBatch / `edu_ctm_batch` | course_code text, trainer_id uuid, start_at timestamptz, end_at timestamptz, capacity int, state enum(PLANNED,OPEN,RUNNING,COMPLETED,MERGED) | PK id; CHECK capacity>0; active/effective partial index | INTERNAL/CONFIDENTIAL; policy retention |
| CompletionRecord / `edu_ctm_completion` | enrollment_id uuid, attendance_percent numeric, assessment_status text, eligible boolean, certificate_document_id uuid? | append-only where history/evidence; UNIQUE(context,enrollment_id) | SENSITIVE_PERSONAL; evidentiary retention |
...
| Entity / table | Domain fields (in addition to baseline) | Constraints / indexes | Sensitivity / retention |
|---|---|---|---|
| Course / `edu_lms_course` | code text, title text, owner_principal_id uuid, version int, state enum(DRAFT,REVIEW,PUBLISHED,RETIRED) | PK id; UNIQUE(context,code,version); index context+status+updated_at | CONFIDENTIAL; learning-record policy |
| CourseEnrollment / `edu_lms_enrollment` | course_id uuid, learner_principal_id uuid, state enum(ENROLLED,IN_PROGRESS,COMPLETED,DROPPED), progress_percent numeric | PK id; UNIQUE(context,course_id,learner_principal_id); index context+parent/reference | CONFIDENTIAL; learning-record policy |
| LearningItem / `edu_lms_item` | course_id uuid, item_type enum(CONTENT,ASSIGNMENT,QUIZ), sequence_no int, title text, document_id uuid?, max_score numeric? | PK id; UNIQUE(context,course_id,sequence_no); active/effective partial index | INTERNAL/CONFIDENTIAL; policy retention |
| SubmissionGrade / `edu_lms_submission` | item_id uuid, learner_principal_id uuid, attempt_no int, document_id uuid?, score numeric?, grade_state enum(SUBMITTED,GRADING,GRADED,RETURNED), graded_by uuid? | append-only where history/evidence; UNIQUE(context,item_id,learner_principal_id,attempt_no) | CONFIDENTIAL; evidentiary retention |
...
| Entity / table | Domain fields (in addition to baseline) | Constraints / indexes | Sensitivity / retention |
|---|---|---|---|
| Exam / `edu_ems_exam` | code text, term_ref uuid, type text, state enum(DRAFT,SCHEDULED,CONDUCTED,EVALUATING,MODERATION,APPROVED,PUBLISHED,CLOSED) | PK id; UNIQUE(context,code); index context+status+updated_at | SENSITIVE_PERSONAL; official-academic-record policy |
| ExamCandidate / `edu_ems_candidate` | exam_id uuid, student_ref uuid, eligible boolean, hold_reason text?, hall_ticket_document_id uuid? | PK id; UNIQUE(context,exam_id,student_ref); index context+parent/reference | SENSITIVE_PERSONAL; official-academic-record policy |
| Evaluation / `edu_ems_evaluation` | exam_id uuid, student_ref uuid, subject_code text, evaluator_id uuid, marks numeric, state enum(DRAFT,SUBMITTED,MODERATED,FINAL), version int | PK id; UNIQUE(context,exam_id,student_ref,subject_code,version); active/effective partial index | INTERNAL/CONFIDENTIAL; policy retention |
| ResultPublication / `edu_ems_result` | exam_id uuid, approval_ref uuid, publication_version int, published_at timestamptz, result_document_ref uuid?, state enum(PREPARED,APPROVED,PUBLISHED,SUPERSEDED) | append-only where history/evidence; UNIQUE(context,exam_id,publication_version) | SENSITIVE_PERSONAL; evidentiary retention |
