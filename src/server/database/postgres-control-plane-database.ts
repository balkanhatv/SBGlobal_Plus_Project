...
/** Fixed Control Plane SQL boundary. It is internal-only and does not itself
 * authorize OperatorElevation use or expose a transport surface. */
export class PostgresControlPlaneDatabase implements SqlDatabase {
  ...
  async transaction<T>(work: (transaction: SqlTransaction) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE sbg_control_plane_rw");
      await client.query("SET LOCAL row_security = on");
      const role = await client.query<{ safe: boolean }>(`SELECT
        current_user = 'sbg_control_plane_rw'
