...
/** Dedicated Notification-worker SQL boundary. It fixes the runtime role to the
 * existing NOBYPASSRLS sbg_notification_worker_rw role from migration 0027. */
export class PostgresNotificationDatabase implements SqlDatabase {
  ...
  async transaction<T>(
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE sbg_notification_worker_rw");
      await client.query("SET LOCAL row_security = on");

      const role = await client.query<{ safe: boolean }>(`SELECT
        current_user = 'sbg_notification_worker_rw'
      ...
      const transaction: SqlTransaction = Object.freeze({
        async query<Row>(
          ...
          if (!active) {
            throw new PostgresDatabaseError("DATABASE_TRANSACTION_CLOSED");
          }
          try {
