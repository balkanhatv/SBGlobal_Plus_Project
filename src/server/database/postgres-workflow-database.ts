...
/** Dedicated Workflow-worker SQL boundary fixed to migration-0027's
 * NOBYPASSRLS sbg_workflow_worker_rw role. */
export class PostgresWorkflowDatabase implements SqlDatabase {
  ...
  async transaction<T>(
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE sbg_workflow_worker_rw");
      await client.query("SET LOCAL row_security = on");

      const role = await client.query<{ safe: boolean }>(`SELECT
        current_user = 'sbg_workflow_worker_rw'
