...
/** Dedicated AI SQL boundary fixed to migration-0014's least-privilege,
 * NOBYPASSRLS sbg_ai_gateway_rw role. */
export class PostgresAIGatewayDatabase implements SqlDatabase {
  ...
  async transaction<T>(
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE sbg_ai_gateway_rw");
      await client.query("SET LOCAL row_security = on");

      const role = await client.query<{ safe: boolean }>(`SELECT
        current_user = 'sbg_ai_gateway_rw'
