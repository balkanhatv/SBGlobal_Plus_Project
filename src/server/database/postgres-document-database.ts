...
/** Dedicated Document-service SQL boundary. It fixes the runtime role to the
 * existing NOBYPASSRLS sbg_document_service_rw role from migration 0028. */
export class PostgresDocumentDatabase implements SqlDatabase {
  ...
  async transaction<T>(
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE sbg_document_service_rw");
      await client.query("SET LOCAL row_security = on");

      const role = await client.query<{ safe: boolean }>(`SELECT
        current_user = 'sbg_document_service_rw'
