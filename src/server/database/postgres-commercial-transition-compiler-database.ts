...
export class PostgresCommercialTransitionCompilerDatabase implements SqlDatabase {
  ...
  async transaction<T>(work:(transaction:SqlTransaction)=>Promise<T>):Promise<T>{
    ...
    try{
      ...
      await client.query("SET LOCAL ROLE sbg_commercial_transition_compiler_rw");
      await client.query("SET LOCAL row_security = on");
      const role=await client.query<{safe:boolean}>(
        "SELECT current_user = 'sbg_commercial_transition_compiler_rw'" +
