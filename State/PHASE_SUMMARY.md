...
**Updated:** 2026-09-23 · **Current checkpoint:** `DEV-AUDIT-EVENT-READ-001`
...
DD-144 adds an exact raw `core_audit.audit_event` reader through the ordinary `RequestScopedSql` application boundary. Final partitioned FORCE-RLS preserves platform/Tenant/Industry/source-target endpoint visibility and raw historical event evidence without creating audit production, search/pagination, retention/export/current-state revalidation or authorization semantics.
