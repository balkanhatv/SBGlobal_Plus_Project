...
Current Development checkpoint is `DEV-COMMERCIAL-ENTITLEMENTS-QUERY-001`. Verified first-party runtime now includes Authorization/Commercial/PEP/audit, idempotency, distributed rate limiting, canonical executor, Zod/tRPC/Fetch, Clerk Bearer auth, trusted web selector/edge controls, pre-context Tenant directory bootstrap, concrete Next.js 15 composition, bounded `core.tenancy.workspace.resolve`, and client-safe `core.commercial.entitlements.getCurrent`. Executable `3bb86bc4b1b313c6bee8c8d65406992bb6b28cc7` / tree `07ebd925d3c0176907ebd257e0cdecb5d0f3840e`: **180 Core + 44 PostgreSQL + 41 migrations / 35 verification files PASS + Next.js 15.5.25 production build/deterministic lock/generated-state cleanliness PASS**. Next is the dependency audit for `core.commercial.subscription.changePlan`; write-side blockers must be closed before the mutation is implemented.
