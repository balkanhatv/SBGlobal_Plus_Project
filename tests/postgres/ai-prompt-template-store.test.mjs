...
import { PostgresAIPromptTemplateStore } from "../../dist/server/ai/postgres-ai-prompt-template-store.js";
...
const role = "sbg_ai_prompt_template_reader_" + randomBytes(8).toString("hex");
const password = randomBytes(24).toString("hex");
const platformCode = "PLATFORM_PROMPT_DD115_" + randomBytes(8).toString("hex");
const serviceCode = "ai-prompt-template-reader-" + randomBytes(8).toString("hex");
...
function contextA(industryContextId = f.industryA1) {
  return Object.freeze({
    ...
    industryContextId,
    dataHomeId: f.home,
    regionCode: "IN-AI-PROMPT-TEMPLATE",
...
before(async () => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO platform_directory.data_home
       ...
       VALUES ($1::uuid,$1::uuid::text,'IN-AI-PROMPT-TEMPLATE','IN','SHARED','ACTIVE')`,
      [f.home],
    );

    for (const [tenantId, primaryIndustry, label] of [
      [f.tenantA, "RTL", "AI PromptTemplate tenant A"],
      [f.tenantB, "EDU", "AI PromptTemplate tenant B"],
    ]) {
      await client.query(
        `INSERT INTO core_tenancy.tenant
         ...
         VALUES ($1::uuid,$1::uuid::text,$2,$2,'ACTIVE',$3,$4::uuid,
           'IN-AI-PROMPT-TEMPLATE',now(),now())`,
        [tenantId, label, primaryIndustry, f.home],
      );
    }

    for (const [principalId, type, label, principalServiceCode, owningModule] of [
      [f.principalA, "HUMAN", "AI PromptTemplate principal A", null, null],
      [f.principalB, "HUMAN", "AI PromptTemplate principal B", null, null],
      [f.platformService, "SERVICE", "AI PromptTemplate platform service",
        serviceCode, "AI"],
    ]) {
      await client.query(
    ...
    await client.query(
      `INSERT INTO core_ai.prompt_template
       ...
       VALUES
        ($1,'INDUSTRY',$6,$8,'ORDER_PROMPT_DD115',2,'Order {{orderId}}',
          '{"required":["orderId"],"type":"object"}'::jsonb,true,ARRAY['tone','locale'],'ACTIVE',$10,NULL,now()-interval '10 days',now()-interval '1 day'),
        ($2,'INDUSTRY',$6,$9,'MAINT_PROMPT_DD115',1,'Maintenance',
          '{"type":"object"}'::jsonb,false,'{}'::text[],'DRAFT',$10,NULL,now()-interval '8 days',now()-interval '2 days'),
        ($3,'TENANT',$6,NULL,'',3,'',
          '{"nested":{"x":1},"type":"object"}'::jsonb,false,ARRAY['','tone','tone'],'RETIRED',$10,NULL,now()-interval '12 days',now()-interval '3 days'),
        ($4,'TENANT',$7,NULL,'TENANT_B_PROMPT_DD115',1,'Tenant B',
          '{"type":"object"}'::jsonb,true,ARRAY['locale'],'ACTIVE',$11,NULL,now()-interval '7 days',now()-interval '1 day'),
        ($5,'PLATFORM',NULL,NULL,$12,4,'Platform governed template',
          '{"type":"object"}'::jsonb,true,ARRAY['locale'],'PUBLISHED',$13,$13,now()-interval '2 days',now()-interval '1 day')`,
      [
        f.promptIndustryA1,
        f.promptIndustryA2,
        f.promptTenantA,
  ...
  store = new PostgresAIPromptTemplateStore(scoped);
});
...
after(async () => {
  ...
  try {
    await client.query("BEGIN");
    await client.query(
      "DELETE FROM core_ai.prompt_template WHERE id=ANY($1::uuid[])",
      [[
        f.promptIndustryA1,
        f.promptIndustryA2,
        f.promptTenantA,
...
test("AIPROMPTTPL-PG-001 exact Industry PromptTemplate preserves immutable raw metadata", async () => {
  const row = await store.loadForContext({
    requestContext: contextA(),
    promptTemplateId: f.promptIndustryA1,
  });

  assert.ok(row);
  assert.equal(row.id, f.promptIndustryA1);
  assert.equal(row.ownerScope, "INDUSTRY");
  assert.equal(row.tenantId, f.tenantA);
  assert.equal(row.industryContextId, f.industryA1);
  assert.equal(row.code, "ORDER_PROMPT_DD115");
  assert.equal(row.version, 2);
  assert.equal(row.systemTemplate, "Order {{orderId}}");
  assert.deepEqual(row.variableSchema, {required: ["orderId"], type: "object"});
  assert.equal(row.groundingRequired, true);
  assert.deepEqual(row.allowedOverrideFields, ["tone", "locale"]);
  assert.equal(row.status, "ACTIVE");
  assert.equal(row.createdBy, f.principalA);
  assert.equal(row.approvedBy, undefined);
  assert.equal(Object.isFrozen(row), true);
  assert.equal(Object.isFrozen(row.variableSchema), true);
  assert.equal(Object.isFrozen(row.allowedOverrideFields), true);
  assert.equal("rendered" in row, false);
  assert.equal("approved" in row, false);
  assert.equal("executable" in row, false);
});

test("AIPROMPTTPL-PG-002 sibling Industry PromptTemplate is hidden", async () => {
  assert.equal(await store.loadForContext({
    requestContext: contextA(),
    promptTemplateId: f.promptIndustryA2,
  }), null);
...
test("AIPROMPTTPL-PG-003 Tenant PromptTemplate is same-Tenant visible and raw values stay raw", async () => {
  const fromIndustry = await store.loadForContext({
    requestContext: contextA(),
    promptTemplateId: f.promptTenantA,
  });
  ...
  assert.equal(fromIndustry.systemTemplate, "");
  assert.deepEqual(fromIndustry.allowedOverrideFields, ["", "tone", "tone"]);
  assert.deepEqual(fromIndustry.variableSchema, {nested: {x: 1}, type: "object"});
  assert.equal(fromIndustry.status, "RETIRED");
  assert.equal(fromTenant.id, f.promptTenantA);
});

test("AIPROMPTTPL-PG-004 PLATFORM PromptTemplate is not Tenant fallback and needs PLATFORM_GLOBAL", async () => {
  assert.equal(await store.loadForContext({
    requestContext: tenantCoreA(),
    promptTemplateId: f.promptPlatform,
  }), null);
  ...
  assert.equal(platform.approvedBy, f.platformService);
  assert.equal("approved" in platform, false);
});

test("AIPROMPTTPL-PG-005 foreign Tenant PromptTemplate is hidden", async () => {
  assert.equal(await store.loadForContext({
    requestContext: tenantCoreA(),
    promptTemplateId: f.promptTenantB,
  }), null);
...
test("AIPROMPTTPL-PG-006 missing, malformed, and route-mismatched reads fail safely", async () => {
  assert.equal(await store.loadForContext({
    requestContext: tenantCoreA(),
    promptTemplateId: f.missingPrompt,
  }), null);

  await assert.rejects(store.loadForContext({
    requestContext: tenantCoreA(),
    promptTemplateId: "not-a-uuid",
  }));
...
test("AIPROMPTTPL-PG-007 raw template evidence adds no selection/approval/render/execute authority and platform writes stay protected", async () => {
  const privileges = await scoped.withContext(platformContext(), (tx) => tx.query(
    `SELECT current_user AS user_name,
            has_table_privilege(current_user,'core_ai.prompt_template','SELECT') AS can_select,
            has_table_privilege(current_user,'core_ai.prompt_template','INSERT') AS can_insert,
            has_table_privilege(current_user,'core_ai.prompt_template','UPDATE') AS can_update,
            has_table_privilege(current_user,'core_ai.prompt_template','DELETE') AS can_delete`,
  ));
  ...
  const attempted = await scoped.withContext(platformContext(), (tx) => tx.query(
    "UPDATE core_ai.prompt_template SET system_template='MUTATED' WHERE id=$1::uuid",
    [f.promptPlatform],
  ));
  ...
  assert.ok(platform);
  assert.equal(platform.systemTemplate, "Platform governed template");
  ...
  assert.equal(typeof store.validateVariables, "undefined");
  assert.equal(typeof store.applyOverrides, "undefined");
