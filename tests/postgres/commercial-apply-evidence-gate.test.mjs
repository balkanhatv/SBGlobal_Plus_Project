...
before(async()=>{
  ...
  try{
    ...
    await c.query("INSERT INTO core_tenancy.tenant(id,tenant_code,legal_name,display_name,status,primary_industry_code,data_home_id,residency_region_code,created_at,updated_at) VALUES ($1::uuid,$1::uuid::text,'Apply gate','Apply gate','PROVISIONING','RTL',$2,'IN-APPLY-GATE',now(),now())",[f.tenant,f.home]);
