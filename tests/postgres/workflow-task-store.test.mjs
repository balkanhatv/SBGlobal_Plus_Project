  ...
  try {
    ...
    await client.query(
      `INSERT INTO core_workflow.workflow_task
       ...
       VALUES
        ($1,$7,$9,$11,'APPROVAL','PRINCIPAL',$13,'workflow.approve','PENDING',
         now()+interval '2 days',NULL,NULL,NULL,0,now()-interval '2 days',now()-interval '3 days'),
        ($2,$7,$9,$11,'REVIEW','ROLE',$15,'','CLAIMED',
         NULL,$13,NULL,NULL,-2,now()-interval '2 days',now()-interval '1 day'),
        ($3,$7,$9,$11,'ACTION','ORG_UNIT',$16,'workflow.action','COMPLETED',
         now()-interval '1 day',$13,$13,now()-interval '12 hours',3,
         now()-interval '3 days',now()-interval '1 day'),
        ($4,$7,$10,$12,'APPROVAL','PRINCIPAL',$13,'workflow.approve','REJECTED',
         NULL,NULL,$13,now()-interval '6 hours',4,now()-interval '2 days',now()-interval '6 hours'),
        ($5,$7,NULL,$13,'REVIEW','PRINCIPAL',$13,'','EXPIRED',
         now()-interval '1 day',NULL,NULL,now()-interval '12 hours',-7,
         now()-interval '4 days',now()-interval '5 days'),
        ($6,$8,NULL,$14,'ACTION','PRINCIPAL',$14,'workflow.cancel','CANCELLED',
         NULL,NULL,$14,now()-interval '2 hours',11,now()-interval '3 days',now()-interval '2 hours')`,
      [
        f.taskPrincipal,
        f.taskRole,
        f.taskOrgUnit,
